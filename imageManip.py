...
def rgb_exclusion(image, channel):
    ...
    Returns:
        out: numpy array of shape(image_height, image_width, 3).
    """
    dictionary_rgb={'R':0,'G':1,'B':2}
    out=image.copy()
    out[:,:,dictionary_rgb.get(channel)]=0*image[:,:,dictionary_rgb.get(channel)]
    return out
...
def lab_decomposition(image, channel):
    ...
    dictionary_rgb={'L':0,'A':1,'B':2}
    out[:,:,dictionary_rgb.get(channel)]=lab[:,:,dictionary_rgb.get(channel)]
    ### YOUR CODE HERE
    ### END YOUR CODE
...
def hsv_decomposition(image, channel='H'):
    ...
    dictionary_hsv={'H':0,'S':1,'V':2}
    out[:,:,dictionary_hsv.get(channel)]=hsv[:,:,dictionary_hsv.get(channel)]
...
def mix_images(image1, image2, channel1, channel2):
    ...
    Returns:
        out: numpy array of shape(image_height, image_width, 3).
    """
    im_left=rgb_exclusion(image1, channel1)
    im_right=rgb_exclusion(image2, channel2)
    combi=np.concatenate((im_left[:,0:150,:],im_right[:,150:,:]), axis=1)
    return combi
...
def mix_quadrants(image):
    ...
    combi=np.concatenate(Top_left[0:150,0:150,:],Top_right[0:150,150:,:],axis=1)
    combi2=np.concatenate(Bottom_left[150:,0:150,:],Bottom_right[150:,150:,:],axis=1)
    out=np.concatenate(combi,combi2,axis=1)
